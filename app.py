...
if analysis:
    ...
    if dates:
        # Clean customer-facing FLOWRIDE chart: one state flow only.
        # Green BUY = confirmed new flow, blue RIDE = trend continuation,
        # red EXIT = confirmed flow break. No repeated BUY/EXIT markers.
        ...
        fsl = slice(max(0, len(dates)-flow_days), len(dates))
        fd = pd.DataFrame({
            "Date": pd.to_datetime(dates[fsl]),
            "Price": chart.get("price", [None]*len(dates))[fsl],
            "Flow": chart.get("flow_line", [None]*len(dates))[fsl],
            "BUY": chart.get("buy", [None]*len(dates))[fsl],
            "EXIT": chart.get("exit", [None]*len(dates))[fsl],
        }).set_index("Date")
        flow_fig = go.Figure()
        flow_fig.add_trace(go.Scatter(x=fd.index, y=fd.Price, mode="lines", name="Price", line=dict(color="#9aa0a6", width=1.5)))
        flow_fig.add_trace(go.Scatter(x=fd.index, y=fd.Flow, mode="lines", name="RIDE", line=dict(color="#1976d2", width=3)))
        flow_fig.add_trace(go.Scatter(x=fd.index, y=fd.BUY, mode="markers", name="BUY", marker=dict(color="#16a34a", size=13, symbol="triangle-up", line=dict(color="white", width=1))))
        flow_fig.add_trace(go.Scatter(x=fd.index, y=fd.EXIT, mode="markers", name="EXIT", marker=dict(color="#dc2626", size=13, symbol="triangle-down", line=dict(color="white", width=1))))
        flow_fig.update_layout(height=420, hovermode="x unified", xaxis_title="Date", yaxis_title="Price", xaxis_rangeslider_visible=False, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0))
        st.plotly_chart(flow_fig, use_container_width=True, config={"displayModeBar": False})
        st.caption("One confirmed BUY starts the flow. Blue RIDE continues through the trend. One confirmed EXIT ends it. No repeated BUY/EXIT signals during the same flow.")
