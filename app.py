...
APP_VERSION = "V8.6D-STOCKS-RIDES-EXIT-CAUTION-FILTER"
...
if analysis:
    ...
    if state == "EXIT":
        ...
        # During EXIT, remove bullish/supportive items that the backend may
        # incorrectly place inside the generic `cautions` list.  Otherwise the
        # UI can say both "Price below short-term trend" and
        # "Price above short-term trend" at the same time.
        supportive_terms = (
            "price above short-term trend",
            "price above medium-term trend",
        ...
        exit_cautions = [
            x for x in cautions
            if not any(term in str(x).lower() for term in supportive_terms)
        ]

        if exit_cautions:
            st.markdown(
                "**Current cautions:**  \n"
                + "  \n".join([f"⚠️ {x}" for x in exit_cautions[:3]])
            )
