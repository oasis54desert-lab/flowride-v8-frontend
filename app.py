...
APP_VERSION = "V9.2-STOCKS-RIDES+RIDE-MANAGER-V3.1+EARLY-WATCH-V4+PROFIT-PROTECTION-V5+WINNER-PATH-V6+DYNAMIC-WINNER-MANAGER-V7.1-FAST-RESEARCH"
...
st.divider()
st.subheader("⚡ Dynamic Winner Manager V7.1 FAST")
st.caption(
    "Dedicated research only: RSI 60–72 CONTROL TIGHT vs STRUCTURAL DYNAMIC. "
    "Older V4/V5/V6 research matrices are skipped. Production FLOWRIDE is unchanged."
)

v71a, v71b, v71c, v71d = st.columns(4)
with v71a:
    v71_exchange = st.selectbox("V7.1 Exchange", ["NSE", "BSE"], key="v71_exchange")
with v71b:
    v71_limit = st.selectbox("V7.1 Stocks", [25, 50, 100, 250, 500], index=0, key="v71_limit")
with v71c:
    v71_period = st.selectbox("V7.1 Period", ["1y", "2y", "5y"], index=1, key="v71_period")
with v71d:
    v71_seed = st.number_input("V7.1 Seed", value=89, step=1, key="v71_seed")

if st.button("⚡ Run V7.1 FAST", use_container_width=True, key="run_v71_fast"):
    try:
        with st.spinner("Running only the V7.1 winner-manager experiment…"):
            v71_result = call(
                "/v7_1_fast",
                {
                    "exchange": v71_exchange,
                    "limit": int(v71_limit),
                    "period": v71_period,
                    "sample_seed": int(v71_seed),
                },
                timeout=900,
            )
        st.session_state["v71_fast_result"] = v71_result
        st.session_state["v71_fast_params"] = {
            "exchange": v71_exchange,
            "limit": int(v71_limit),
            "period": v71_period,
            "sample_seed": int(v71_seed),
        }
        st.success("V7.1 FAST completed.")
    except Exception as e:
        st.error(f"V7.1 FAST failed: {e}")

_v71r = st.session_state.get("v71_fast_result")
_v71p = st.session_state.get("v71_fast_params")
if _v71r:
    _v71summary = _v71r.get("summary", {}) or {}
    _v71variants = _v71summary.get("variants", []) or []
    if _v71variants:
        st.dataframe(pd.DataFrame(_v71variants), use_container_width=True, hide_index=True)

    with st.expander("V7.1 FAST diagnostics"):
        st.json(_v71r.get("diagnostics", {}))

    if _v71p:
        try:
            _sum_csv = call_bytes("/v7_1_fast_summary_csv", _v71p, timeout=120)
            st.download_button(
                "⬇️ Download V7.1 FAST Summary CSV",
                _sum_csv,
                file_name=f"flowride_v7_1_fast_{_v71p['exchange']}_{_v71p['period']}_seed_{_v71p['sample_seed']}.csv",
                mime="text/csv",
                use_container_width=True,
            )
            _trade_csv = call_bytes("/v7_1_fast_trades_csv", _v71p, timeout=120)
            st.download_button(
                "🔬 Download V7.1 FAST Trade-Level CSV",
                _trade_csv,
                file_name=f"flowride_v7_1_fast_trades_{_v71p['exchange']}_{_v71p['period']}_seed_{_v71p['sample_seed']}.csv",
                mime="text/csv",
                use_container_width=True,
            )
        except Exception as e:
            st.warning(f"V7.1 FAST CSV not ready: {e}")
