...
st.caption("V8.9 uses representative deterministic sampling and measures what happens after every historical FLOWRIDE BUY signal.")
...
if st.button("🧪 RUN STRATEGY VALIDATION", type="primary", use_container_width=True):
    try:
        ...
        st.caption("V9.0 keeps per-stock failures in the skipped list; if the whole request still fails, check the backend log entry for the exact endpoint error.")
...
st.caption("V9.0 applies a stricter BUY quality gate (trend, momentum, ADX, volume and volatility), filters incomplete/illiquid histories, then ranks actual BUY/RIDE setup quality.")
