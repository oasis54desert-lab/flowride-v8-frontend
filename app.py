...
APP_VERSION = "V8.6A-STOCKS-RIDES-EXPLAINABLE-FLOW"
...
if analysis:
    ...
    st.success(f"**{f.get('primary_action', '—')}** • {f.get('lifecycle', state)}")
    ...
    if state == "EXIT":
        reasons = f.get("exit_reason", []) or []
        if reasons: st.warning("**Exit explanation:** " + " • ".join(reasons))
