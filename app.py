...
APP_VERSION = "V9.1-SEPARATE-FRONTEND"
...
st.caption("V9.2 tests a deterministic representative sample from the expanded exchange universe and measures what happens after every historical FLOWRIDE BUY signal.")

ub1, ub2, ub3, ub4, ub5 = st.columns(5)
with ub1:
    ub_exchange = st.selectbox("Exchange", ["NSE", "BSE"], key="ub_exchange")
with ub2:
    ub_limit = st.selectbox("Stocks to test", [25, 50, 100, 250, 500, 1000, 2000, 5000, "ALL"], index=2, key="ub_limit")
with ub3:
...
if st.button("🧪 RUN STRATEGY VALIDATION", type="primary", use_container_width=True):
    try:
        with st.spinner(f"Testing {ub_limit if ub_limit == 'ALL' else 'up to ' + str(ub_limit)} representative {ub_exchange} stocks and measuring 5D / 10D / 20D BUY outcomes..."):
            st.session_state.flowride_universe_backtest = call(
                "/universe_backtest",
                {
                    "exchange": ub_exchange,
                    "limit": 0 if ub_limit == "ALL" else int(ub_limit),
                    "period": ub_period,
        ...
        st.caption("V9.2 keeps per-stock failures in the skipped list; the results remain visible for stocks that completed successfully.")
...
if ub:
    ...
    requested = int(su.get("Requested Stocks", 0) or 0)
    completed = int(su.get("Successfully Tested", 0) or 0)
    skipped = int(su.get("Failed / Skipped", 0) or 0)
    st.caption(f"Requested: {requested} • Successfully tested: {completed} • Failed/skipped: {skipped} • Sample seed: {int(su.get('Sample Seed', 89) or 89)}")

    def horizon_success_text(h):
        rate = su.get(f"{h}D Signal Success Rate %")
        wins = int(su.get(f"{h}D Winning Signals", 0) or 0)
        tested_h = int(su.get(f"{h}D Signals Tested", 0) or 0)
        if rate is None or tested_h <= 0:
            return "—", "No valid forward signals"
        return f"{float(rate):.1f}%", f"{wins}/{tested_h} winning signals"

    h5_value, h5_delta = horizon_success_text(5)
    h10_value, h10_delta = horizon_success_text(10)
    h20_value, h20_delta = horizon_success_text(20)
    ...
    c.metric("5D Success", h5_value, h5_delta)
    d.metric("10D Success", h10_value, h10_delta)
    e.metric("20D Success", h20_value, h20_delta)
    ...
    d.metric("Profitable Beat Buy & Hold", f"{float(su.get('Beat Buy & Hold %', 0) or 0):.1f}%")
    e.metric("Avg Profitable Outperformance", f"{float(su.get('Average Outperformance %', 0) or 0):+.2f}%")
    ...
    if results.empty:
        ...
        st.caption("Ranking prioritizes 20-day BUY-signal success and return. Outperformance is counted only when FLOWRIDE itself is profitable and beats Buy & Hold; relative difference is shown separately.")
    ...
    with st.expander("ℹ️ How to read the success rate", expanded=False):
        ...
        st.write("• The x/y shown below each success metric is the exact number of winning signals divided by the number of signals with enough future data for that horizon.")
...
st.caption("V9.2 applies a stricter BUY quality gate (trend, momentum, ADX, volume and volatility), filters incomplete/illiquid histories, then ranks actual BUY/RIDE setup quality.")
...
if st.button("🔄 RUN EOD SCAN", type="primary", use_container_width=True):
    try:
        ...
        if df.empty:
            ...
            rename = {"Symbol":"symbol","Company":"name","Exchange":"exchange","Close":"close","Score":"score","Signal":"signal","FLOWRIDE":"flowride","Flow Score":"flow_score","Setup Quality":"setup_quality","Quality Gate":"quality_gate","Gate Gaps":"gate_gaps","RSI":"rsi","ATR %":"atr_pct"}
...
if "scan" in st.session_state:
    ...
    if not df.empty:
        ...
        cols = [c for c in ["symbol","name","exchange","close","score","signal","flowride","flow_score","setup_quality","quality_gate","gate_gaps","rsi","atr_pct"] if c in df.columns]
