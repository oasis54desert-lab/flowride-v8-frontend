...
st.info(
    "Upload the V17 trades CSV. Recommended starting test: ₹10,00,000 capital, "
    "10 maximum simultaneous positions, 10% position size, 0.25% base round-trip cost."
)

v18_upload = st.file_uploader(
    "Upload V17 trades CSV for V18",
    type=["csv"],
    key="v18_trades_upload"
)
...
if st.button("🧪 Run V18 Stress & Portfolio Validation", use_container_width=True, key="run_v18"):
    if v18_upload is None:
        st.error("Upload the V17 trades CSV first.")
    else:
        try:
            import requests as _v18_requests
            _v18_files = {
                "file": (
                    v18_upload.name,
                    v18_upload.getvalue(),
            ...
            _v18_data = {
                "starting_capital": str(v18_capital),
                "max_positions": str(int(v18_max_positions)),
                "position_size_pct": str(v18_position_pct),
                "base_cost_pct": str(v18_base_cost),
            }
            _v18_headers = {}
            try:
                if API_KEY:
                    _v18_headers["x-api-key"] = API_KEY
            except Exception:
                pass

            with st.spinner("Running V18 portfolio and stress tests..."):
                _v18_resp = _v18_requests.post(
                    API_BASE.rstrip("/") + "/v18/analyze",
                    files=_v18_files,
                    data=_v18_data,
                    headers=_v18_headers,
                    timeout=300,
                )
            ...
            if _v18_resp.status_code != 200:
                ...
                st.success("V18 completed. Previous FLOWRIDE settings remain unchanged.")
...
if _v18_res:
    ...
    st.dataframe(
        pd.DataFrame(_v18_res.get("cost_stress", [])),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.dataframe(
        pd.DataFrame(_v18_res.get("extreme_winner_removal", [])),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.dataframe(
        pd.DataFrame(_v18_res.get("extreme_winner_caps", [])),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.dataframe(
        pd.DataFrame(_v18_res.get("winner_concentration", [])),
        use_container_width=True,
        hide_index=True,
    )
    ...
    if v18_upload is not None:
        try:
            ...
            try:
                if API_KEY:
                    _v18_headers2["x-api-key"] = API_KEY
            except Exception:
                pass
            _v18_csv = _v18_requests.post(
                API_BASE.rstrip("/") + "/v18/export_csv",
                files=_v18_files2,
                data=_v18_data2,
                headers=_v18_headers2,
                timeout=300,
            )
