...
APP_VERSION = "V8.9-STOCKS-RIDES-VALIDATION+ASYNC-BATCH-VALIDATION+EARLY-WATCH-V4"
    ...
    if results.empty:
        ...
        try:
            if research_csv:
                ...
                if "Hit +10% within 20D" in v4df.columns:
                    hit10 = v4df["Hit +10% within 20D"].astype(str).str.lower().isin(["true","1","yes"])
                else:
                ...
                show_cols = [c for c in [
                    "Symbol", "Setup Date", "Setup Close", "Confirm 5D Close Return %",
                    "EarlyWatchV4Day5ReturnPct", "V4 Result", "Hit +10% within 20D",
                    remaining_col
