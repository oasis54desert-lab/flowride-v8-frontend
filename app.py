...
APP_VERSION = "V8.3-STOCKS-RIDES-CURRENT-STATE-CLARITY"
...
if analysis:
    ...
    if dates:
        st.markdown("### 🌊 Stocks Rides — FLOW HISTORY")
        st.caption("Current state is shown above. This chart shows historical flow transitions: 🟢 BUY starts a ride and 🔴 EXIT ends that ride. A historical EXIT marker does not mean the current state is EXIT.")
        ...
        if len(exx): fig.add_trace(go.Scatter(x=exx, y=ey, mode="markers+text", name="EXIT (HISTORICAL)", marker=dict(size=13, symbol="triangle-down"), text=["EXIT"]*len(exx), textposition="bottom center"))
