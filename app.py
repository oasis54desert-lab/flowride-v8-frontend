...
APP_VERSION = "V8.8-STOCKS-RIDES-UNIVERSE-BACKTEST"
...
st.divider()
st.subheader("🌐 FLOWRIDE Universe / Batch Backtest")
st.caption("Validate the same FLOWRIDE strategy across multiple stocks automatically. Start with 25 stocks before testing larger batches.")
ub1,ub2,ub3,ub4=st.columns(4)
with ub1: ub_exchange=st.selectbox("Exchange",["NSE","BSE"],key="ub_exchange")
with ub2: ub_limit=st.selectbox("Stocks to test",[25,50,100],index=0,key="ub_limit")
with ub3: ub_period=st.selectbox("Period",["1y","2y","5y"],index=1,key="ub_period")
with ub4: ub_capital=st.number_input("Capital per stock (₹)",min_value=1000.0,max_value=100000000.0,value=100000.0,step=10000.0,key="ub_capital")
if st.button("🚀 RUN UNIVERSE BACKTEST",type="primary",use_container_width=True):
    try:
        with st.spinner(f"Running FLOWRIDE on up to {ub_limit} {ub_exchange} stocks. Large batches may take time..."):
            st.session_state.flowride_universe_backtest=call("/universe_backtest",{"exchange":ub_exchange,"limit":int(ub_limit),"period":ub_period,"initial_capital":float(ub_capital)},timeout=900)
    except Exception as e: st.error(f"Universe backtest failed: {e}")
ub=st.session_state.get("flowride_universe_backtest")
if ub:
    st.markdown(f"### 📊 {ub.get('exchange','NSE')} Universe Results — {ub.get('period','2y')}")
    su=ub.get("summary",{}) or {}
    m1,m2,m3,m4,m5=st.columns(5)
    m1.metric("Stocks Tested",int(su.get("Successfully Tested",0) or 0)); m2.metric("Beat Buy & Hold",f"{float(su.get('Beat Buy & Hold %',0) or 0):.1f}%"); m3.metric("Avg FLOWRIDE",f"{float(su.get('Average FLOWRIDE Return %',0) or 0):+.1f}%"); m4.metric("Avg Buy & Hold",f"{float(su.get('Average Buy & Hold Return %',0) or 0):+.1f}%"); m5.metric("Avg Outperformance",f"{float(su.get('Average Outperformance %',0) or 0):+.1f}%")
    m1,m2,m3,m4=st.columns(4)
    m1.metric("Total Trades",int(su.get("Total Completed Trades",0) or 0)); m2.metric("Positive FLOWRIDE",f"{float(su.get('Positive FLOWRIDE %',0) or 0):.1f}%"); m3.metric("Avg Max Drawdown",f"{float(su.get('Average Max Drawdown %',0) or 0):.1f}%"); m4.metric("Avg Win Rate",f"{float(su.get('Average Win Rate %',0) or 0):.1f}%")
    results=pd.DataFrame(ub.get("results",[]) or [])
    if results.empty: st.warning("No stocks completed successfully. Check backend logs and market-data availability.")
    else:
        st.markdown("### 🏆 FLOWRIDE Universe Ranking"); st.dataframe(results,use_container_width=True,hide_index=True)
        chart_df=results.sort_values("FLOWRIDE Return %",ascending=True); fig_ub=go.Figure(); fig_ub.add_trace(go.Bar(x=chart_df["FLOWRIDE Return %"],y=chart_df["Symbol"],orientation="h",name="FLOWRIDE Return %")); fig_ub.update_layout(title="FLOWRIDE Return by Stock",height=max(420,min(1400,28*len(chart_df)+120)),margin=dict(l=10,r=10,t=50,b=10),xaxis_title="Return %",yaxis_title="Stock"); st.plotly_chart(fig_ub,use_container_width=True,config={"displayModeBar":False})
        st.download_button("⬇️ Download Universe Backtest CSV",results.to_csv(index=False).encode("utf-8"),file_name=f"flowride_universe_backtest_{ub.get('exchange','NSE')}_{ub.get('period','2y')}.csv",mime="text/csv",use_container_width=True)
    failures=pd.DataFrame(ub.get("failures",[]) or [])
    if not failures.empty:
        with st.expander(f"⚠️ Failed / skipped stocks ({len(failures)})",expanded=False): st.dataframe(failures,use_container_width=True,hide_index=True)
    with st.expander("⚙️ Universe backtest assumptions",expanded=False):
        for item in ub.get("assumptions",[]) or []: st.write(f"• {item}")
